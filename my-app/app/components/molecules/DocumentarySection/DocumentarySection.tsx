...
const DocumentaryItem = ({ title, subtitle, year, text }: DocumenttaryItemProps) => {
    return (
        <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.1 }} className='relative w-full cursor-pointer border-[0.3px] border-[rgba(255,184,0,1)] rounded-2xl bg-linear-to-b from-black to-black/0 backdrop-blur-sm px-8 py-6 flex md:items-center gap-20 max-xl:gap-10 justify-between  text-white max-md:flex-col'>
            <div className='flex flex-col justify-between flex-1 shrink-0'>
                ...
                <p className='md:px-2 font-against opacity-33 text-[16px] max-xl:text-[12px] mt-8 max-md:mt-4'>{year}</p>
            </div>
