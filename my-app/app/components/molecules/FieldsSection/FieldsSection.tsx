...
const FieldItem = ({item}: {item: string}) => {
    return (
        <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.1 }} className='cursor-pointer border-[0.4px] border-[rgba(255,184,0,1)] rounded-sm bg-linear-to-b from-black to-black/0 backdrop-blur-sm p-2 font-sans text-center text-[20px] max-2xl:text-[16px] opacity-60 text-[#FFFFFF]'>
            {item}
        </motion.div>
    )
}
...
const FieldsSection = ({ref}: any) => {
    return (
        <div ref={ref} className='w-full scroll-mt-24'>
            ...
            <div className='grid grid-cols-3 gap-5 max-2xl:gap-3 max-lg:grid-cols-2 max-sm:grid-cols-1'>
