...
const AboutSection = ({ref}: any) => {
  return (
    <div ref={ref} className='scroll-mt-16 relative w-full border-[0.4px] border-[#FFB800] rounded-2xl flex justify-center py-20 overflow-hidden max-sm:p-6'>
        ...
        <motion.img initial={{y: 100}} whileInView={{y:0}} transition={{duration: 1, ease: 'anticipate', delay: 0.1}} viewport={{once: true, amount: 0.2}} src="base.svg" alt="base" className='absolute z-10 bottom-0'/>
        <motion.img initial={{y: 100}} whileInView={{y:0}} transition={{duration: 1, ease: 'anticipate', delay: 0.2}} viewport={{once: true, amount: 0.2}} src="base2.svg" alt="base" className='absolute z-10 bottom-0'/>
        <motion.img initial={{y: 100}} whileInView={{y:0}} transition={{duration: 1, ease: 'anticipate', delay: 0.3}} viewport={{once: true, amount: 0.2}} src="base3.svg" alt="base" className='absolute z-10 bottom-0'/>
        <motion.img initial={{y: 100}} whileInView={{y:0}} transition={{duration: 1, ease: 'anticipate', delay: 0.4}} viewport={{once: true, amount: 0.2}} src="base4.svg" alt="base" className='absolute z-10 bottom-0'/>
    </div>
  )
}
